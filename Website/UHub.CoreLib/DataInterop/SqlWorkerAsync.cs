...
namespace UHub.CoreLib.DataInterop
{
    ...
    internal static partial class SqlWorker
    {
        ...
        /// <exception cref="SqlException"/>
        /// <exception cref="ObjectDisposedException"/>
        public static async Task<IEnumerable<T>> ExecBasicQueryAsync<T>(string SqlConn, string CmdName, Action<SqlCommand> SetParams = null) where T : DBEntityBase
        {
                ...
                using (var cmd = new SqlCommand(CmdName, conn))
                {
                    ...
                    try
                    {
                        using (var reader = await cmd.ExecuteReaderAsync())
                        {
                            ...
                            {
                                returnSet.Add((T)(dynamic)reader);
                            }
        ...
        /// <exception cref="SqlException"/>
        /// <exception cref="ObjectDisposedException"/>
        public static async Task<IEnumerable<T>> ExecBasicQueryAsync<T>(string SqlConn, string CmdName, Func<SqlDataReader, T> ReturnValParseFunc)
        {
